...
import { constants, selectors, sagamain, sagas } from 'routes/ChoiceAs/modules/choiceas'
// import { expandedLog, deepDiffMapper } from 'util/debug/tools'

t('should export a function checkRequire', (a) => {
  a.equal(typeof sagas.checkRequire, 'function', 'is a function')

  a.throws(() => sagas.checkRequire({one: ''}, {}),
    /expected action.one to exist/, 'should throw error on missing require')

  a.throws(() => sagas.checkRequire({one: /^[a-z]$/}, {one: 'ab2c'}),
    /expected action.one to match /, 'should throw error on non-match regex')

  a.throws(() => sagas.checkRequire({one: ''}, {one: 'ab2c'}),
    /undefined/, 'should not throw')

  a.throws(() =>
    sagas.checkRequire({one: {two: /^[a-z]$/}}, {one: {two: 'ab2c'}}),
...
t('should export a function wrapHandler', (a) => {
  a.equal(typeof sagas.wrapHandler, 'function', 'function exported')
  ...
  const handler = sagas.wrapHandler({handler: makeTestHandler(), errorHandler})
  ...
  const badHandler = sagas.wrapHandler({
    handler: makeTestHandler(),
    errorHandler,
    require: {one: ''},
...
t('should export a sagamain that forks all handlers', (a) => {
  a.equal(typeof sagamain, 'function', 'function exported')
  ...
  const iterator = sagamain({hldrs: testHandlers, wrapper: testWrapper})
